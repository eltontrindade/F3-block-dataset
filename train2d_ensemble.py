...
def main(save_path=cfg.save, 
    ...
    for i in range(3):
        ...
        os.makedirs(os.path.join(save_path,'canal_{}'.format(canal)),exist_ok = True)
...
def train(model, train_set, test_set, save, valid_set, n_epochs):
    ...
    with open(os.path.join(save,'canal_{}'.format(canal), 'logs.csv'), 'w') as f:
        f.write('epoch,')
    ...
    for epoch in range(n_epochs):
        os.makedirs(os.path.join(cfg.save,'canal_{}'.format(canal), 'epoch_{}'.format(epoch)),exist_ok = True)
        grafico={}
        ...
        if cfg.save_all:
            torch.save(model.state_dict(), os.path.join(save,'canal_{}'.format(canal), 'epoch_{}'.format(epoch), 'model.dat'))
        if log_dict['test_iou'] > best_iou:
            ...
            print('New best dice: %.4f' % log_dict['test_iou'])
            
        else:
            print('Current best dice: %.4f' % best_iou)
