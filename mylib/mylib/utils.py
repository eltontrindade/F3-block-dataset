...
def copy_file_backup(save):
    ...
    shutil.copytree(os.path.join(sys.path[1], '../', 'mylib'), os.path.join(backup_dir, 'mylib'))
    shutil.copytree(os.path.join(sys.path[1], '../../', 'acsconv'), os.path.join(backup_dir, 'acsconv'))
    os.makedirs(os.path.join(backup_dir, 'current_experiment'))
    for file_path in os.listdir(sys.path[1]):
        if file_path not in ['tmp', 'data', '__pycache__']:
            shutil.copy(os.path.join(sys.path[1], file_path), os.path.join(backup_dir, 'current_experiment'))
