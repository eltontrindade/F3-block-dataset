...
class _EncoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, dropout=False):
        ...
        layers = [
            nn.Conv3d(in_channels, out_channels,groups=2, kernel_size=[7,1,7], padding=[3,0,3]),
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True),
            nn.Conv3d(out_channels, out_channels,groups=2, kernel_size=[7,1,7], padding=[3,0,3]),
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True),
        ]
        if dropout:
...
class _DecoderBlock(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels):
        ...
        self.decode = nn.Sequential(
            nn.Conv3d(in_channels, middle_channels, groups=2,kernel_size=[7,1,7], padding=[3,0,3]),
            nn.BatchNorm3d(middle_channels),
            nn.ReLU(inplace=True),
            nn.Conv3d(middle_channels, out_channels,groups=2, kernel_size=[7,1,7], padding=[3,0,3]),
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True),
        )
